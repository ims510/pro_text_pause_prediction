...
######################Prediction on randomised pauses positions###################
# Load the randomized texts from a separate folder (change this to your folder path)
# randomized_texts = get_texts("/Users/madalina/Documents/M1TAL/stage_GC/randomised_fichiersavecpausescatRan")

# # Prepare the dataset for the randomized texts (same process as for training texts)
# randomized_dataset = prepare_training_data(randomized_texts)
# print(f"Randomized test dataset size: {len(randomized_dataset)}")

# # Function to evaluate model on the randomized test dataset
# def evaluate_randomized_texts(randomized_dataset):
...
#     for batch in randomized_dataset:
#         inputs = {k: torch.tensor(v).unsqueeze(0) for k, v in batch.items() if k != 'labels'}
#         labels = torch.tensor(batch['labels']).unsqueeze(0)
...
#     print(f"Accuracy for 'pause' on randomized texts: {pause_accuracy:.2f}")
#     print(f"Accuracy for 'no pause' on randomized texts: {no_pause_accuracy:.2f}")
...
# # Run evaluation on the randomized texts
# print("\nEvaluating on Randomized Texts:")
# randomized_accuracy = evaluate_randomized_texts(randomized_dataset)
# print("\nRandomized Test Accuracy:", randomized_accuracy)
