...
csv_file = "/Users/madalina/Documents/M1TAL/stage_GC/pro_text/classifications/all_tagged_decale.csv"
...
def extract_features(burst_data):
    ...
    pauseDur = round(float(burst_data[2]), 2)  # This is the target we want to predict
...
def remove_negative_values(df):
    ...
    for index in range(len(pauses_df)):
        ...
        new_line = [float(num) for num in new_line if num != ""]
        for num in new_line:
            if num < 0:
                new_line.remove(num)
        
        pauses_df[index] = str(new_line)
    return df
...
def remove_long_pauses(df):
    ...
    df = df[df["pauseDur"] >= abs(lower_bound)]
...
def main():
    ...
    print("removed long pauses")
    vectorise_text(df)
    print("vectorised text")
    df.to_csv('vectorised_data.csv')
    print("made vectorised_data.csv")
    df_expanded = expand_columns(df)
    print("expanded columns")
    df_expanded.to_pickle('expanded_data_decale.pkl')
    print("made pickle")
    return df_expanded
